        ...
        DI: AsyncWriteOnlyDataCommand,
    {
        match self {
            ...
            Command::EnableScroll(en) => {
                Self::send_commands(iface, &[0x2E | (en as u8)]).await
            }
            ...
            Command::AddressMode(mode) => {
                Self::send_commands(iface, &[0x20, mode as u8]).await
            }
            ...
            Command::PageStart(page) => {
                Self::send_commands(iface, &[0xB0 | (page as u8)]).await
            }
            Command::StartLine(line) => {
                Self::send_commands(iface, &[0x40 | (0x3F & line)]).await
            }
            ...
            Command::DisplayOffset(offset) => {
                Self::send_commands(iface, &[0xD3, offset]).await
            }
